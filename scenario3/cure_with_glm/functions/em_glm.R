em_glm <- function(t_obs, deltas, X, lam0, beta0, xies_start, gammas_start, tol = 1e-6,maxit=1000, j=1){
  #
  env<-new.env()
  n <- length(t_obs)
  ...
  y <- c(y1,y0)
  #
  X_glm <- as.data.frame(rbind(X_glm,X_glm))
  #
  lliks<-c()
  ...
  while ((dif > tol) & (iter<maxit)) {
    ...
    # Update pies
    w <- c(z,1-z)
    glm_fit <-glm(y~sex + age + period + socio_low + socio_mid + socio_high + log_tms,
       data = X_glm, family = binomial(), weights = w) # link logit
    #pies <- as.vector(glm_fit$coefficients)
    prob0 <- glm_fit$fitted.values[1:n] # probability of recovery
