em_splines <- function(t_obs, deltas, X, lam0, beta0, xies_start, gammas_start, tol = 1e-6,maxit=1000, j=1){
  ...
  X_gam <- as.data.frame(rbind(X_gam,X_gam))
  #
  lliks<-c()
  ...
  while ((dif > tol) & (iter<maxit)) { 
    ...
    tryCatch(
      {
        assign("gam_fit", gam(y ~ sex + s(age) + s(period) + s(log_tms) + socio_low + socio_mid + socio_high +
                              ti(age,period) + ti(age,log_tms) + ti(period,log_tms), 
                              family = binomial(), 
